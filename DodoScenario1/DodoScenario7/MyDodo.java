...
public class MyDodo extends Dodo
{
    ...
    public void act() {
        if (myNrOfStepsTaken < Mauritius.MAXSTEPS) {
            ...
            ((Mauritius) getWorld()).updateScore(Mauritius.MAXSTEPS - myNrOfStepsTaken, myScore);
        }
    }
    ...
    public void moveRandomly() {
        ...
        while (myNrOfStepsTaken < Mauritius.MAXSTEPS) {
            ...
            if (canMove()) {
                move();
                myNrOfStepsTaken++;
                ((Mauritius) getWorld()).updateScore(Mauritius.MAXSTEPS - myNrOfStepsTaken, myNrOfStepsTaken);
            }
        }
    }
    ...
    public Egg getBestEgg(List<Egg> lijst) {
        Egg bestEgg = null;
        double bestScore = -1;
            for (Egg egg : lijst) {
                int distance = getDistance(egg);
                ...
                double scorePerStep = (double) value / distance;
                if (scorePerStep > bestScore)   {
                    bestScore = scorePerStep;
                    bestEgg = egg;
                }
            }
        return bestEgg;
    ...
    public void pickUpNearestEggInList() {
        ...
        if (nearest instanceof GoldenEgg) {
            myScore += 5;
        } else {
            myScore += 1;
        }
